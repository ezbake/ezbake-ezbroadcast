...
public abstract class EzBroadcaster implements Closeable {
    ...
    public void unsubscribeFromTopic(String topic) {
        topicKeys.remove(topic.toLowerCase());
        unsubscribe(topic);
    }
    ...
    public void broadcast(String topic, Visibility visibility, byte[] payload) throws IOException {
        ...
        if (isProduction) {
            try {
                ...
                RSAKeyCrypto crypto = topicKeys.get(topic.toLowerCase()); 
        ...
        try {
            if (message.isSetKey()) {
                if (crypto != null && crypto.hasPrivate()) {
                    ...
                    if (evaluator.evaluate(new ColumnVisibility(visibility.getFormalVisibility()))) {
    ...
    /**
     * This method should only be overridden in test situations. This gets the authorizations
     * for the broadcaster. This implementation of the method will be used in production.
     *
     * @param props Properties object that contains security properties
     */
    protected VisibilityEvaluator getVisibilityEvaluator(Properties props) {
        EzbakeSecurityClient client = new EzbakeSecurityClient(props);
        try {
            EzSecurityToken token = client.fetchAppToken();
            Set<String> authsList = token.getAuthorizations().getFormalAuthorizations();
            Authorizations broadcasterAuths = new Authorizations(authsList.toArray(new String[authsList.size()]));
            log.info("Broadcaster initialized with auths: " + broadcasterAuths.toString());
            return new VisibilityEvaluator(broadcasterAuths);
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
    }
    ...
    public static EzBroadcaster create(Properties props, String groupId) {
        ...
        try {
            ...
            if (broadcaster.isProduction) {
                broadcaster.evaluator = broadcaster.getVisibilityEvaluator(props);
            } else {
    ...
    public static EzBroadcaster create(Properties props, String groupId, String key, String topic, boolean isPrivate) {
        ...
        try {
            broadcaster.topicKeys.put(topic.toLowerCase(), new RSAKeyCrypto(key, isPrivate));
    ...
    protected void loadTopicKey(String topic, boolean isListener) {
        if (isProduction) {
            String topicName = topic.toLowerCase();

            // If the key has been loaded, do not load again.
            if (topicKeys.containsKey(topicName)) {
                log.warn("The key for topic [{}] has already been loaded.", topic);
                return;
            }

            String privateFile = "keys/" + topicName + ".priv";
            String publicFile = "keys/" + topicName + ".pub";
            ...
            if (privateInputStream == null && publicInputStream == null) {
                log.error("Did not find any keys for topic [" + topicName + "].");
                throw new IllegalStateException("Did not find any keys for topic [" + topicName + "].");
            }

            // Listeners need private key. If no private key was found, we need to fail pipeline.
            if (isListener && privateInputStream == null) {
                throw new IllegalStateException("Did not find private key for listener topic [" + topicName + "].");
            }
            ...
            if (privateInputStream != null) {
                ...
                throw new IllegalStateException("Encountered an unexpected state for [" + topicName + "].");
            }
        }
    }
